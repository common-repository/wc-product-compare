 ...
 * WC_Session.
 */
class WCPC_Session extends WC_Session {
	...
	public function __construct() {
	    ...
	    $this->_cookie = 'wcpc_session_' . COOKIEHASH;
		...
		if ( $cookie = $this->get_session_cookie() ) {
			...
			if ( time() > $this->_session_expiring ) {
				...
				$session_expiry_option = 'wcpc_session_expires_' . $this->_customer_id;
	...
	public function set_customer_session_cookie( $set ) {
		if ( $set ) {
			...
			// Set the cookie
			wc_setcookie( $this->_cookie, $cookie_value, $this->_session_expiration, apply_filters( 'wcpc_session_use_secure_cookie', false ) );
		}
	}
	...
	public function set_session_expiration() {
		$this->_session_expiring   = time() + intval( apply_filters( 'wcpc_session_expiring', 60 * 60 * 47 ) ); // 47 Hours.
		$this->_session_expiration = time() + intval( apply_filters( 'wcpc_session_expiration', 60 * 60 * 48 ) ); // 48 Hours.
	}
	...
	public function save_data() {
		...
		if ( $this->_dirty && $this->has_session() ) {
			...
			$session_option        = '_wcpc_session_' . $this->_customer_id;
            $session_expiry_option = '_wcpc_session_expires_' . $this->_customer_id;
	...
	public function destroy_session() {
		// Clear cookie
		wc_setcookie( $this->_cookie, '', time() - YEAR_IN_SECONDS, apply_filters( 'wcpc_session_use_secure_cookie', false ) );
	...
	public function cleanup_sessions() {
		...
		if ( ! defined( 'WP_SETUP_CONFIG' ) && ! defined( 'WP_INSTALLING' ) ) {
            ...
            foreach ( $wc_session_expires as $option_name ) {
                ...
                $expired_sessions[] = "_wcpc_session_$session_id"; // Session key
            }
	...
	public function get_session( $customer_id, $default = false ) {
		...
		return get_option( '_wcpc_session_' . $customer_id);
	}
	...
	public function delete_session( $customer_id ) {
		// Delete session
        $session_option        = '_wcpc_session_' . $customer_id;
        $session_expiry_option = '_wcpc_session_expires_' . $customer_id;
